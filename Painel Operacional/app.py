...
st.markdown(
    ...
    .kpi-value-yellow { font-size: 32px; font-weight: 800; color: #fbc02d; margin-top: 6px; }
    .kpi-value-orange { font-size: 32px; font-weight: 800; color: #f57c00; margin-top: 6px; }
...
def get_status_e_cor(perc):
  if perc <= 40.0:
    ...
    return "REGULAR", "#fbc02d", "badge-regular"
  else:
...
def render_barra_empilhada_torre(v_doca, v_trans, v_cheg, v_fin, chart_key):
  ...
  fig.add_trace(
      go.Bar(
          ...
          insidetextanchor="middle",
          textfont=dict(color="#000000", size=14, family="sans-serif"),
          marker=dict(color="#00e676", cornerradius=10),  # Verde (EM DOCA / NO PRAZO)
      )
  )
  fig.add_trace(
      go.Bar(
          ...
          insidetextanchor="middle",
          textfont=dict(color="#000000", size=14, family="sans-serif"),
          marker=dict(color="#fbc02d", cornerradius=10),  # Amarelo (EM TRANSITO / ATR ORIG)
      )
  )
  fig.add_trace(
      go.Bar(
          ...
          insidetextanchor="middle",
          textfont=dict(color="#ffffff", size=14, family="sans-serif"),
          marker=dict(color="#f57c00", cornerradius=10),  # Laranja (CHEGADA DESTINO / ATR DEST)
      )
  )
  fig.add_trace(
      go.Bar(
          ...
          insidetextanchor="middle",
          textfont=dict(color="#ffffff", size=14, family="sans-serif"),
          marker=dict(color="#ff5252", cornerradius=10),  # Vermelho (FINALIZADO / NO SHOW)
      )
  )
  fig.update_layout(
      barmode="stack",
...
def render_torre_multi():
        ...
        <div class="charts-card">
            <div class="charts-header">
                ...
                <div class="charts-legend">
                    ...
                    <span style="color:#fbc02d;">● EM TRANSITO</span>
                    <span style="color:#f57c00;">● CHEGADA DESTINO</span>
  ...
  for item in st.session_state.dados_torre:
    ...
    perc = (doca / qtd * 100) if qtd > 0 else 0.0
    dados_proc_torre.append({
        "CLIENTE": item.get("CLIENTE", ""),
        "REALIZADAS": doca,
        "PENDENTES": max(0, qtd - doca),
        "%": perc,
    })
  ...
  h4.markdown(
      '<div class="th-title" style="text-align:center;"><span'
      ' style="color:#fbc02d;">●</span> EM TRANSITO</div>',
      unsafe_allow_html=True,
  )
  h5.markdown(
      '<div class="th-title" style="text-align:center;"><span'
      ' style="color:#f57c00;">●</span> CHEGADA DESTINO</div>',
      unsafe_allow_html=True,
  )
  h6.markdown(
      '<div class="th-title" style="text-align:center;"><span'
