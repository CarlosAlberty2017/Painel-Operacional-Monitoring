...
# Renderização da Aba TORRE MULTI GERENCIAL (com os novos campos)
def render_torre_multi_gerencial():
    tot_qtd = sum(item.get("QTD", 0) for item in st.session_state.dados_torre_gerencial)
    tot_no_prazo = sum(
        item.get("NO_PRAZO", 0) for item in st.session_state.dados_torre_gerencial
    )
    tot_atr_orig = sum(
        item.get("ATR_ORIG", 0) for item in st.session_state.dados_torre_gerencial
    )
    tot_atr_dest = sum(
        item.get("ATR_DEST", 0) for item in st.session_state.dados_torre_gerencial
    )
    tot_no_show = sum(
        item.get("NO_SHOW", 0) for item in st.session_state.dados_torre_gerencial
    )
    ...
    for idx, item in enumerate(st.session_state.dados_torre_gerencial):
        col_cli, col_qtd, col_prazo, col_atrorig, col_atrdest, col_noshow, col_perc = (
            st.columns([2.2, 1.0, 1.0, 1.1, 1.1, 1.0, 2.0])
        )
        ...
        with col_qtd:
            ...
            if v != qtd:
                ...
                salvar_dados("dados_torre_gerencial", st.session_state.dados_torre_gerencial)
                st.rerun()
